import hrequests  # Make sure you have this installed or change to `requests` if `hrequests` is a typo
...
def scrape_jobs(page_number):
    headers = {'User-Agent': get_random_agent()}  # Use a new random User-Agent for each request
    ...
    for job in jobs:
        ...
        if table.fetchone() is not None:
            print(f"Job already exists in the database: {job_title}")
            continue
